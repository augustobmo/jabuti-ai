...
messages = [
    {
        ...
        "content":"Você é uma boneca da Branca de Neve. Você vai estar falando com crianças em português. Faça a responsta em 20 palavras ou menos. Se for um assunto complexo, peça se quer saber mais. Depois de confirmar, você pode dar a resposta completa. Se for um conteúdo adulto, despiste."
    },
]
...
while True:
    for message in live_speech():
        ...
        if detect_wakeup(message, wakeup_words):
            ...
            sound_path = str(Path(__file__).parent / "sounds" / "detected.mp3")
            playsound.playsound(sound_path)
            #playsound.playsound(Path(__file__).parent / "sounds" / "detected.mp3")
            break
    for message in live_speech(50):
        sound_path = str(Path(__file__).parent / "sounds" / "processing.mp3")
        playsound.playsound(sound_path)
